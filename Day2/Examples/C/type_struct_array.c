...
int main(int argc, char **argv){
  ...
  if(myRank==0){
    ...
    ierror=MPI_Type_size(newtype, &bufsize);
    ierror=MPI_Type_get_extent(newtype, &buflb, &bufextent);
    ierror=MPI_Type_get_true_extent(newtype, &buflb, &buftrueextent);
  ...
  for( i = 0; i < uniSize; i++) {
    ierror=MPI_Isend(&sBuf, 2, newtype, dest, 100, MPI_COMM_WORLD, &request); 
    ierror=MPI_Recv(&rBuf, 2, newtype, src, 100, MPI_COMM_WORLD, &status);  
