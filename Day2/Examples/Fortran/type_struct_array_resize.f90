...
program typestructresize
  ...
  TYPE(MPI_Datatype) :: array_of_types(2), newtype, newtype_resized
  integer :: bufsize
  ...
  CALL MPI_Get_address(sBuf(1), dbladdress)
  CALL MPI_Get_address(sBuf(2), intaddress)
  bufextent = MPI_Aint_diff(intaddress, dbladdress)
  buflb=0
  CALL MPI_Type_create_resized(newtype, buflb, bufextent, newtype_resized)
  call MPI_Type_commit(newtype_resized, ierror)
  ...
  do i=1, uniSize
    call MPI_Isend(sBuf, 2, newtype_resized, dest, 100, MPI_COMM_WORLD, request, ierror)
    call MPI_Recv(rBuf, 2, newtype_resized, src, 100, MPI_COMM_WORLD, status, ierror)
  ...
  call MPI_Type_free(newtype_resized, ierror)

  call MPI_Finalize(ierror)
end program typestructresize
